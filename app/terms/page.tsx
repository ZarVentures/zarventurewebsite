...
import termsContent from "@/data/content/terms.json"

export const metadata = {
  title: "Terms and Conditions - Zar Ventures",
  description: "Terms and Conditions for Zar Ventures Real Estate",
}
            ...
            <div className="max-w-4xl mx-auto space-y-8">
              <div className="bg-card rounded-lg border border-border shadow-lg p-6">
                {content.intro.paragraphs.map((paragraph, index) => (
                  <p key={index} className={`text-foreground/80 leading-relaxed ${index === 0 ? 'mb-4' : ''}`}>
                    {paragraph}
                  </p>
                ))}
              </div>
              ...
              {content.sections.map((section) => (
                <div key={section.id} className="bg-card rounded-lg border border-border shadow-lg p-6">
                  ...
                  {section.items && (
                    <ul className={`space-y-2 text-foreground/80 ${section.additionalContent ? 'mb-4' : ''}`}>
